...
nstatAr = nstates*ones(1, length(temperature)); % # of states used for Z can vary with T

%% pnrem 
premMaxSame = zeros(length(gAr), length(temperature));
premMaxDiff = zeros(length(gAr), length(temperature));
...
%}
%% analytic version
Zdiffa = zeros(length(gAr), length(temperature), length(nbAr));
Ediffa = zeros(length(gAr), length(temperature), length(nbAr));
for j = 1:length(gAr)
    g = gAr(j);
    for k = 1:length(nbAr)
        nbar = nbAr(k);
...
Zsame = zeros(length(gAr), length(temperature), length(nbAr));
Esame = zeros(length(gAr), length(temperature), length(nbAr));
for j = 1:length(gAr)
    g = gAr(j);
    for k = 1:length(nbAr)
        nbar = nbAr(k);
...
WAr = zeros(length(gAr), length(temperature), length(nbAr));
...
if length(gAr) == length(temperature) == length(nbAr)== 1
    save(char(strcat('thermalSZE_InteractingBosons_slurm_workspace_gAr_', string(gAr), '_TAr_', string(temperature), '_nbAr_', string(nbAr))))
else
    save(char(strcat('/workspace/thermalSZE_InteractingBosons_slurm_workspace_gAr_', string(gAr(1)), '_TAr_', string(temperature(1)), '_nbAr_', string(nbAr(1)))))
end
fprintf(WAr);
