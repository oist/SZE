...
function[] = densPlot(nbar)
np = 2^8;
figure(1)
subplot(2,2,1);
psi1 = analyticNbar(nbar,1, np);

subplot(2,2,3);
psi2 = analyticNbar(nbar,2, np);
...
subplot(2,2,[2,4]); imagesc(probArray);
return
hold on;
cd 'Documents/OIST/sze_out/matlab/'
filename = char(strcat('densPlot_nbar',string(nbar), '.png'))
