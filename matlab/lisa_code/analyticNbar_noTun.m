...
%well
function [psiSplit]= analyticNbar_noTun(nbar, n, np)
    ...
    plot(xvalues,psiInf); hold on;
    
  
    
    %new WF construction starts
    ...
    %get prob for gaps in infwell state
    pGap = zeros((nbar+1), 1);
    for i=1:(nbar+1)
        pGap(i) = sum(psiInf(indexBar(i):indexBar(i+1)).^2)/norm(psiInf);
    end
    %normalization error!!
    %for now: renormalize pgap!
    pGap = pGap/sum(pGap);
    ...
    for i = 1:(nbar+1)
        ...
        psiSplit(indexBar(i):indexBar(i+1)) = thisPsi*pGap(i);
    end
    norm(psiInf)
    norm(psiSplit)
